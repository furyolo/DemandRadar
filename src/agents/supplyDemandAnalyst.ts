  ...
  const response = await options.llm.generateJson(SupplyDemandAnalysisResponseSchema, [
    {
      ...
      content: [
        ...
        'creator_capability_fit.status must be direct, orchestrate, or not_fit. Explain the exact work the creator can own and the exact capability gaps.',
        'existing_supply_fit.status must be sufficient, partial, missing, or unknown. Separate named existing tools/providers/workarounds from unresolved gaps.',
        'ai_agent_fill.feasibility must be high, medium, or low. List concrete automatable steps in can_do and concrete boundaries in cannot_do.',
        'third_party_supply_path must state whether a third party is needed, the provider type, why, and the handoff boundary.',
